...
## Theme and welcome screen
Per a later request to match the visual convention of national health-portal apps (e.g. eSanjeevani), `src/config.js`'s primary color shifted from teal to a deep institutional blue (`#0B5CAB`), matching the website's theme, with a saffron accent (`#E67E00`) added for primary calls-to-action. A `WelcomeScreen` now sits before `Login` in the navigation stack — a proper intro screen (headline, three feature highlights, language switcher, a saffron "Get Started" button) rather than dropping a first-time user straight into a login form, mirroring the website's new landing page. I could not access the live eSanjeevani site directly to screenshot it (it blocks automated requests) — this is built from the well-established Indian government health-portal design convention, not a pixel-accurate copy of the current live site.
...
- **Bhashini voice read-aloud is unverified against the live API, but now has a working fallback that doesn't depend on it.** `ReadAloudButton` tries Bhashini first when credentials are configured (real synthesized speech, better regional-language pronunciation), and falls back to the device's own built-in TTS via `expo-speech` when credentials aren't set yet *or* when a live Bhashini call fails for any reason (network, bad credentials, an API shape mismatch). This matters because Bhashini requires registering at https://bhashini.gov.in/ulca/user/register and waiting for approval, which takes time you may not have before a demo — the device-TTS fallback means the feature works today, and upgrades to real Bhashini audio automatically the moment credentials are added to `src/config.js`, with no further code changes. Caveat: device TTS quality and Hindi/Marathi voice-pack availability depend on what's installed on the actual phone — test on your real target devices, not just one.
